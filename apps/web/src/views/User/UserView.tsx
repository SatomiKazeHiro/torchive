...
function UserAvatar({
  ...
  if (url) {
    return (
      <img
        ...
        alt="avatar"
        className={cn(
          `${cls} 2xs-soft rounded-full border-4 border-card bg-card object-cover`,
          className,
        )}
      />
    );
  ...
  return (
    <div
      className={cn(
        `${cls} 2xs-soft flex items-center justify-center rounded-full border-4 border-card bg-zinc-900 font-bold text-white dark:bg-zinc-700`,
        className,
      )}
    >
      {initial}
...
function UserView() {
  ...
  return (
    <div id="user-page" className="min-h-screen bg-page font-sans">
      {/* 顶部导航栏 - 返回首页等极简操作 */}
      <header className="fixed top-0 right-0 left-0 z-50 border-b border-edge-subtle bg-card/80 backdrop-blur-md transition-all">
      ...
      <div className="pt-14">
        {/* 用户信息区和导航 */}
        <div className="2xs-soft mb-6 border-b border-edge-subtle bg-card">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                >
                  ...
                  <div className={nameHeightClasses["xl"]}>
                    ...
                    <div className="flex h-1/2 flex-col justify-center">
                      <h1 className="line-clamp-1 text-xl font-bold tracking-tight text-primary sm:text-2xl">
                        {displayName}
                      </h1>
                      <div className="mt-1 flex items-center gap-3">
                        <p className="text-xs text-muted sm:text-sm">UID: {userId}</p>
                      </div>
                    </div>
                    {/* 用户签名/简介 */}
                    <div className="mt-4 line-clamp-2 text-xs text-secondary sm:text-sm">
                      <p>这是你的个人主页，在这里管理收藏、历史记录以及稍后再看。</p>
                    </div>
                    {/* 数据统计 */}
                    <div className="mt-2 flex items-center gap-4 text-xs whitespace-nowrap text-muted sm:text-sm">
                    ...
                    return (
                      <Link
                        ...
                        className={cn(
                          ...
                          isActive ? "font-bold text-primary" : "text-muted hover:text-secondary",
                        )}
                      >
                        <item.icon
                          size={18}
                        ...
                        {item.name}
                        {isActive && (
                          <span className="absolute bottom-0 left-1/2 h-1 w-8 -translate-x-1/2 rounded-t-full bg-accent"></span>
                        )}
                      </Link>
                    );
                  })}
