...
import type { SearchResult, SearchState, ThemeMode } from "../types";
...
export default function SearchDrawer({
  ...
  useEffect(() => {
    if (isOpen && activeResultRef.current && searchState.results.length > 0) {
      setTimeout(() => {
        activeResultRef.current?.scrollIntoView({ behavior: "auto", block: "center" });
      }, 100);
    }
  ...
  const handleResultClick = (result: SearchResult) => onResultSelect(result);
  ...
  const renderHighlightedText = (content: string, matches: Array<{ start: number; end: number }>) => {
    ...
    if (lastEnd < content.length) parts.push({ text: content.slice(lastEnd), isHighlight: false });

    return parts.map((part, i) =>
      part.isHighlight ? (
        <mark key={i} className={cn("rounded px-0.5", theme.highlight)}>{part.text}</mark>
      ) : (
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className={cn(
            "fixed right-0 top-0 z-[60] h-full w-96 border-l shadow-2xs",
            theme.bg,
            theme.border
          )}
        >
          ...
          <div className={cn("border-b px-4 py-3", theme.border)}>
            <div className="flex items-center justify-between">
              ...
              <button onClick={onClose} className={cn("rounded p-1.5 transition-colors", theme.text, theme.hover)}>
              ...
              <div className="flex gap-2">
                <div className="relative flex-1">
                  <input
                    ...
                    placeholder="输入关键词（至少2字）..."
                    className={cn(
                      "h-9 w-full rounded border px-3 pr-8 text-sm outline-none transition-colors focus:border-zinc-400",
                      theme.input
                    )}
                  />
                  {localKeyword && (
                    <button
                      type="button"
                      onClick={handleClear}
                      className={cn("absolute right-2 top-1/2 -translate-y-1/2 rounded p-0.5 opacity-50 hover:opacity-100", theme.text)}
                    >
                ...
                <button
                  ...
                  className={cn(
                    "flex h-9 items-center justify-center rounded border px-3 text-sm font-medium transition-colors disabled:cursor-not-allowed disabled:opacity-40",
                    themeMode === "dark"
            ...
            {searchState.results.length > 0 && (
              <div className={cn("mt-3 flex items-center justify-between text-sm", theme.text)}>
                <span>{searchState.currentResultIndex + 1} / {searchState.results.length}</span>
                <div className="flex gap-1">
                  <button onClick={onPrevResult} disabled={searchState.currentResultIndex <= 0} className={cn("rounded p-1.5 transition-colors disabled:opacity-30", theme.hover)}>
                    <BiChevronUp className="h-4 w-4" />
                  </button>
                  <button onClick={onNextResult} disabled={searchState.currentResultIndex >= searchState.results.length - 1} className={cn("rounded p-1.5 transition-colors disabled:opacity-30", theme.hover)}>
                    <BiChevronDown className="h-4 w-4" />
                  </button>
                  <button onClick={onClearSearch} className={cn("ml-2 rounded p-1.5 transition-colors text-zinc-500 hover:text-red-500", theme.hover)}>
          ...
          <div className="h-[calc(100%-140px)] overflow-y-auto p-2 scrollbar-thin">
            {searchState.isSearching ? (
              ...
              <div className={cn("p-4 text-center text-sm", theme.text)}>未找到 &quot;{searchState.keyword}&quot; 相关内容</div>
            ) : searchState.results.length === 0 ? (
              <div className={cn("p-4 text-center text-sm opacity-60", theme.text)}>
                {localKeyword.trim().length === 1 ? "请至少输入2个字符进行搜索" : "输入关键词开始搜索"}
              </div>
            ) : (
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        theme.border,
                        isActive ? cn(theme.active, "border-zinc-400") : cn(theme.resultBg, theme.hover)
                      )}
                    >
