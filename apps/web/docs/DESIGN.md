...
### HarmonyOS Sans SC — Primary brand font for all UI text, headings, and body. Its neutral, technical-feel Latin glyphs paired with native CJK coverage keep the UI crisp across both scripts without font fallback noise. · `--font-harmonyos-sans-sc`
- **Weights:** 400, 500, 700 (Bold)
...
- **Role:** Primary brand font for all UI text, headings, and body. Loaded via `@font-face` from `src/assets/fonts/HarmonyOS_Sans_SC_Regular.woff2` (declared in `src/index.css`).

### System Mono — Used for code snippets or specific input fields requiring monospaced characters. Reinforces a technical aesthetic. · `--font-geist-mono`
...
:root {
  ...
  --font-harmonyos-sans-sc: 'HarmonyOS Sans SC', ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif;
